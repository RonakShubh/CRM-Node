  ...
  try {
    ...
    let filteredResult = result.map((item) => ({
      ...
      fields: item?.fields?.filter(
        (field) => !field.isDefault && !field.delete
      ),
    }));
    ...
    return res
      .status(400)
      .json({ success: false, message: "Failed", error: err.toString() });
  }
};
  ...
  try {
    ...
    if (collectionExists) {
      ...
      if (existingSchema) {
        ...
        const caseInsensitiveMatch = Object.keys(existingSchema.paths)?.find(
          (path) => {
            return (
              path.toLowerCase() === fieldName.toLowerCase() ||
              path.toLowerCase() === labelName.toLowerCase()
            );
          }
        );
        const caseInsensitiveMatchlabel = Object.keys(
        ...
        const existingField = await CustomField.findOne({
          ...
          "fields.name": {
            $regex: new RegExp(`^${req.body?.fields[0]?.name}$`, "i"),
          },
        });
        ...
        const existingLabel = await CustomField.findOne({
          _id: req.body.moduleId,
          "fields.label": {
            $regex: new RegExp(`^${req.body?.fields[0]?.label}$`, "i"),
          },
        });
        ...
        if (caseInsensitiveMatch || existingField) {
          return res.status(409).json({
            success: false,
            message: `Field with Name '${fieldName}' already exists`,
          });
        } else if (caseInsensitiveMatchlabel || existingLabel) {
          return res.status(409).json({
            success: false,
            message: `Label Name ${labelName} already exists`,
          });
        }
        ...
        if (updateResult?.nModified > 0) {
          ...
          return res
            .status(404)
            .json({ success: false, message: "Failed to add fields" });
        }
      } else {
        return res.status(404).json({
          success: false,
          message: "Model/schema not found for collection",
        });
      }
    } else {
      return res
...
const editWholeFieldsArray = async (req, res) => {
  try {
    ...
    if (result?.nModified > 0) {
      return res
        .status(200)
        .json({ message: "Fields updated successfully", result });
    } else {
      return res
        .status(404)
        .json({ success: false, message: "Failed to update fields", result });
    }
  } catch (err) {
...
const editSingleField = async (req, res) => {
  try {
    ...
    const customFieldBeforeUpdate = await CustomField.findOne({
      _id: moduleId,
    ...
    if (collectionExists) {
      ...
      if (existingSchema) {
        const existingField = customFieldBeforeUpdate.fields.find(
        ...
        const updatedFieldBcType =
          req.body?.updatedField?.backendType || "Mixed";
        const isFieldNameChanged =
          updatedFieldName && updatedFieldName !== existingField.name;
        ...
        if (isFieldNameChanged) {
          ...
          nameAlreadyExists = isDuplicateName(
            updatedFieldName,
            customFieldBeforeUpdate?.fields?.map((field) => field.name)
          );

          // Check if the updatedFieldName exists in the schema
          const existingFieldInSchema = Object.keys(existingSchema.paths);
          if (
            isDuplicateName(updatedFieldName, existingFieldInSchema) &&
            updatedFieldName.toLowerCase() !== existingField.name.toLowerCase()
          ) {
            return res.status(409).json({
              success: false,
              message: "Duplicate name found in schema",
            });
          }
        }
        ...
        if (updateResult?.nModified > 0) {
          const selectedField = customFieldBeforeUpdate.fields.find(
            (field) => field._id.toString() === fieldId
          );
          ...
          return res.status(200).json({
            success: true,
            message: "No changes made, document already up-to-date",
          });
        } else {
          return res
            .status(404)
      ...
      if (collectionExists) {
        ...
        if (existingSchema) {
          ...
          if (updatedCustomField) {
            ...
            return res
              .status(200)
              .json({ message: "Field removed successfully" });
          } else {
            return res
              .status(400)
              .json({ success: false, message: "Failed to remove field" });
          }
        } else {
          return res.status(404).json({
            success: false,
        ...
        return res
          .status(404)
          .json({ success: false, message: "Collection not exists" });
      }
    } else {
      return res
        .status(404)
...
const deleteManyFields = async (req, res) => {
  try {
      ...
      if (collectionExists) {
        ...
        if (existingSchema) {
          ...
          if (updateResult?.nModified > 0) {
            ...
            return res
              .status(200)
              .json({ message: "Fields removed successfully" });
          } else {
            return res
              .status(404)
              .json({ success: false, message: "Failed to remove fields" });
          }
        } else {
          return res.status(404).json({
            success: false,
    ...
    return res.status(404).json({
      success: false,
      message: "Failed to remove fields",
      error: err.toString(),
    });
  }
};
...
const createNewModule = async (req, res) => {
  try {
    ...
    const url = req?.body?.isDefault
      ? ""
      : req.protocol + "://" + req?.get("host");
    const file = `${url}/api/custom-field/icon/${req?.file?.filename}`;

    const existingModule = await CustomField.findOne({
    ...
    if (existingModule) {
      return res
        .status(400)
        .json({ success: false, message: `Module name not available !` });
    }
    ...
    if (!mongoose.models[moduleName]) {
      ...
      return res
        .status(400)
        .json({ success: false, message: `Module name already exist` });
    }
    ...
    return res
      .status(200)
      .json({ message: "Module added successfully", data: newModule });
  } catch (err) {
...
const changeIcon = async (req, res) => {
  try {
    ...
    if (!req?.file) {
      return res
        .status(400)
        .send({ success: false, message: "No icon uploaded." });
    }
    ...
    if (existingDocument) {
      ...
      let result = await CustomField.findByIdAndUpdate(
        { _id: id },
        { $set: { icon: file } },
        { new: true }
      );
    ...
    return res
      .status(400)
      .json({ success: false, message: "Failed to update icon" });
  }
};
  ...
  try {
    ...
    const changeModuleName = async (oldModuleName, newModuleName) => {
      ...
      if (await mongoose.models[oldModuleName]) {
        ...
        console.error(`Module ${oldModuleName} does not exist`);
      }
    };
    ...
    return res.status(200).json({
      message: `Module name changed from ${oldModule?.moduleName} to ${moduleName}`,
      result,
    });
  } catch (err) {
    console.error("Failed to create module:", err);
...
const addHeading = async (req, res) => {
  try {
    ...
    if (existingField) {
      const headingExists = existingField.headings.some((heading) =>
        req.body.headings[0].heading.includes(heading.heading)
      );
...
const editSingleHeading = async (req, res) => {
  try {
    ...
    if (result?.nModified > 0) {
      return res
        .status(200)
        .json({ message: "Heading updated successfully", result });
    } else {
      return res
        .status(404)
        .json({ success: false, message: "Failed to update heading", result });
    }
  } catch (err) {
    console.error("Failed to Update Custom Heading:", err);
    return res.status(400).json({
      success: false,
      message: "Failed to Update Custom Heading",
      error: err.toString(),
    });
  }
};
...
const editWholeHeadingsArray = async (req, res) => {
  try {
    let result = await CustomField.updateOne(
      { _id: req.params.id },
      { $set: { headings: req.body } }
    );

    if (result?.nModified > 0) {
      return res
        .status(200)
        .json({ message: "Headings updated successfully", result });
    } else {
      return res
        .status(404)
        .json({ success: false, message: "Failed to update headings", result });
    }
  } catch (err) {
    console.error("Failed to Update Custom Headings:", err);
    return res.status(400).json({
      success: false,
      message: "Failed to Update Custom Headings",
      error: err.toString(),
    });
  }
};
...
const deleteHeading = async (req, res) => {
  try {
    ...
    const result = await CustomField.updateOne(
      ...
      {
        arrayFilters: [{ "elem.belongsTo": headingId }],
      }
    );
    ...
    if (result?.nModified > 0) {
      return res
        .status(200)
        .json({ message: "Heading removed successfully", result });
    } else {
      return res
        .status(404)
        .json({ success: false, message: "Failed to remove heading", result });
    }
  } catch (err) {
...
const deleteManyHeadings = async (req, res) => {
  try {
    ...
    const result = await CustomField.updateOne(
      ...
      {
        arrayFilters: [{ "elem.belongsTo": { $in: headingsIds } }],
      }
    );
    ...
    if (result?.nModified > 0) {
      return res
        .status(200)
        .json({ message: "Headings removed successfully", result });
    } else {
      return res
        .status(404)
        .json({ success: false, message: "Failed to remove headings", result });
    }
  } catch (err) {
    console.error("Failed to delete headings ", err);
    return res.status(404).json({
      success: false,
      message: "Failed to remove headings",
      error: err.toString(),
    });
  }
};
...
const changeIsTableField = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: successMessage, result });
  } catch (err) {
    console.error("Failed to change table field status:", err);
    return res.status(400).json({
      message: "Failed to change table field status : ",
      error: err.toString(),
    });
  }
};
...
const changeIsTableFields = async (req, res) => {
  try {
    ...
    if (!updates || !Array.isArray(updates)) {
      return res
        .status(400)
        .json({ success: false, message: `Invalid 'updates' format` });
    }
    ...
    const updateOperations = updates.map(({ fieldId, isTableField }) => ({
      updateOne: {
        filter: {
          _id: moduleId,
          "fields._id": new mongoose.Types.ObjectId(fieldId),
        },
    ...
    if (!updateResult) {
      return res
        .status(404)
        .json({ success: false, message: "Fields not found" });
    }

    if (updateResult?.nModified > 0) {
      return res
        .status(200)
        .json({ success: true, message: "Updated successfully", updateResult });
      ...
      return res
        .status(400)
        .json({ success: true, message: "Failed to update", updateResult });
    }
  } catch (err) {
...
const deletmodule = async (req, res) => {
  try {
    const module = await CustomField.findByIdAndUpdate(req.params.id, {
      deleted: true,
    });
...
const deleteManyModule = async (req, res) => {
  try {
    const module = await CustomField.updateMany(
      { _id: { $in: req.body } },
      { $set: { deleted: true } }
    );
    res
      .status(200)
      .json({ message: "Many module delete successfully", module });
  } catch (err) {
    res.status(404).json({ message: "error", err });
  }
};
...
const changeFieldsBelongsTo = async (req, res) => {
  try {
    ...
    const headingExists = await CustomField.findOne({
      _id: moduleId,
      "headings._id": headingId,
    });

    if (!headingExists) {
      return res
        .status(404)
        .json({ success: false, message: "Module or Heading not found" });
    }
    ...
    const updateOperations = updates?.map((update) => ({
      updateOne: {
        filter: {
          _id: moduleId,
          "fields._id": new mongoose.Types.ObjectId(update.fieldId),
        },
        update: { $set: { "fields.$[field].belongsTo": update.belongsTo } },
        arrayFilters: [
          { "field._id": new mongoose.Types.ObjectId(update.fieldId) },
        ],
      },
    }));
