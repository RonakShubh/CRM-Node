...
const addAuthImg = async (req, res) => {
  try {
    ...
    return res.send({
      data: newImgDocument,
      message: "Auth image uploaded successfully",
    });
  } catch (err) {
    ...
    return res
      .status(400)
      .json({ success: false, message: "Failed to add auth image" });
  }
};
...
const addAuthAndLogoImg = async (req, res) => {
  try {
    if (
      ...
      return res
        .status(400)
        .send({ success: false, message: "No auth file uploaded" });
      ...
      return res
        .status(400)
        .send({ success: false, message: "No logo files uploaded" });
    }
    ...
    if (logoLgImg && logoSmImg) {
      ...
      smFile = `${url}/api/images/logoImg/${
        logoSmImg ? logoSmImg?.[0]?.filename : logoLgImg?.[0]?.filename
      }`;
      lgFile = `${url}/api/images/logoImg/${
        logoSmImg ? logoSmImg?.[0]?.filename : logoLgImg?.[0]?.filename
      }`;
    }

    const newImgDocument = new Img({
      authImg: authFile,
      logoSmImg: smFile,
      logoLgImg: lgFile,
      createdDate,
    ...
    await newImgDocument.save();
    await Img.updateMany(
      { _id: { $ne: newImgDocument._id } },
      { $set: { isActive: false } }
    );
    return res.send({
      data: newImgDocument,
...
const UpdateAuthImg = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ success: false, message: "Failed to update auth Image" });
  }
};

const changeLogoImg = async (req, res) => {
  // add, edit
  try {
    const { id } = req.params;
    ...
    if (existingDocument) {
      ...
      await Img.updateOne(
        { _id: id },
        { $set: { logoSmImg: smFile, logoLgImg: lgFile } }
      );
      return res.send({
        smLogoPath: smFile,
        lgLogoPath: lgFile,
    ...
    return res
      .status(400)
      .json({ success: false, message: "Failed to set logo imges" });
  }
};
...
const updateAuthAndLogoImg = async (req, res) => {
  try {
    ...
    if (existingDocument) {
      ...
      let result = await Img.findByIdAndUpdate(
        { _id: id },
        { $set: { ...updateData } },
        { new: true }
      );
    ...
    return res
      .status(400)
      .json({ success: false, message: "Failed to update images" });
  }
};
...
const setActiveImg = async (req, res) => {
  try {
    ...
    const imageData = await Img.findByIdAndUpdate(
      req.params.id,
      { isActive: isActive },
      { new: true }
    );
